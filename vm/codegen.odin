...
codegen_clear_labels      :: proc() 
{ 
	if len(current_codegen.labels) > 0 do clear(&current_codegen.labels)
	current_codegen.ldepth = 0
}
...
// Nota(jstn): somente para arwuivos funções
codegen_init_main_cache :: proc()
{
	cache       := ctx_new(Cache) 
	cache.ints   = ctx_make(map[int]int)
	cache.floats = ctx_make(map[f32]int)
	cache.symids = ctx_make(map[int]int)
	current_codegen.caches.current = cache

	nuo_assert(cache != nil,"Something went wrong. Could not create CacheData.")
}

// Nota(jstn): funções normais
codegen_init_cache :: proc()
{
	cache       := ctx_anew(Cache) 
	cache.ints   = ctx_amake(map[int]int)
	cache.floats = ctx_amake(map[f32]int)
	cache.symids = ctx_amake(map[int]int)
	current_codegen.caches.current = cache

	nuo_assert(cache != nil,"Something went wrong. Could not create CacheData.")
}

codegen_get_cache         :: proc "contextless" () -> ^Cache { return current_codegen.caches.current }

codegen_set_cache         :: proc "contextless" (c : ^Cache) { current_codegen.caches.current = c }


codegen_clear_cache       :: proc ()
{
	current := current_codegen.caches.current
	clear(&current.ints)
	clear(&current.floats)
	clear(&current.symids)
}
...
{
	codegen_report_error_cond(node != nil,"something went wrong in generate Bytecode.")               
	codegen_report_error_cond(rcount < max_recursion ,"something went wrong. Max recursion reached.")
	...
	#partial switch node.type
	{
		...
		case .INT:
			...
			cache    := codegen_get_cache()
			int_node := reinterpret_node(node,IntNode)
			
			_int     := int_node.number
			loc      := int_node.loc

			if index, has := cache.ints[_int]; has do codegen_emit_loadi(index,loc)
			else do cache.ints[_int] = codegen_emit_load(VARIANT_VAL(_int),loc)
		...
		case .FLOAT:
			...
			float      := float_node.number
			loc        := float_node.loc

			if index, has := cache.floats[float]; has do codegen_emit_loadi(index,loc)
			else do cache.floats[float] = codegen_emit_load(VARIANT_VAL(float),loc)
		...
		case .COMPLEX:
			...
			codegen_emit_opcode(.OP_CONSTRUCT,loc)
			codegen_emit_index(argc,loc)
			codegen_emit_type(.COMPLEX,loc)

			codegen_report_error_cond(argc == 2,"Complex codegen bug, Please report.",argc)
		...
		case .CALLABLE:
			...
			codegen_emit_opcode(.OP_CONSTRUCT,loc)
			codegen_emit_index(argc,loc)
			codegen_emit_type(.CALLABLE,loc)

			codegen_report_error_cond(argc == 1,"implement Callable's args %v handler",argc)
		...
		case .SET_PROPERTY:
			...
			if index, has := cache.symids[property_sid]; has do sidi = index
			else {
				sidi  = codegen_push_constant(SYMID_VAL(property_sid))
				cache.symids[property_sid] = sidi
			}

			nuo_assert(sidi >= 0, " sidi >= 0 is false! this's a compiler bug. Please report .")
			...
			set_property :: proc(node: ^Node)
			{
				#partial switch node.type
				{
					...
					case : unreachable("there's no set_property handler for '%v'.",node.type)
				}
			}
		...
		case .BLOCK:
			...
			if local_count > 0 { 
				codegen_emit_opcode(.OP_SUB_SP,loc)
				codegen_emit_index(local_count,loc) 
			}
		...
		case .MATCH:
			...
			if clen > 0 || default
			{
				...
				for &case_data in alabels 
				{
					addr          := case_data.start_address
					if index, has := caches.ints[addr]; has do codegen_emit_loadi(index,loc)
					else do codegen_emit_load(VARIANT_VAL(addr),loc)
				}
		...
		case .CLASS:
			...
			properties  := class_node.properties_n
			name        := class_node.name
			super       := class_node.super
			extends     := class_node.extends
			field_count := class_node.field_count 
			loc         := class_node.loc

			class_id    := class_db_register_class(name)

			for property in properties do class_register_property(class_id,property,class_db_default_setter,class_db_default_getter)
		...
		case .IMPORT:
			...
			if native
			{
				...
				if codegen_report_error_cond(_iid >= 0, "cannot resolve '%v' as native Import",name) 
				{
					fn, ok := import_get_init_fn(_iid)
					if !import_db_was_loaded(_iid) && ok { fn(_iid); import_db_set_loaded(_iid,true) }
				}
			...
			else if imported
			{
				...
				nuo_assert( _iid >= 0, " iid >= 0 is false! this's a compiler bug. Please report .")
		...
		case : codegen_report_error_cond(false,"not handled '%v'.",node)
