...
import_stmt :: proc(bp: BindPower) -> ^Node 
{
    ...
    compile_error_cond(ok,"something went wrong in loading '%v'.",import_path_tk.text)
...
class_stmt :: proc(bp: BindPower) -> ^Node
{
	...
	/*
		Class XYZ
		{
			set foo = nil is compiled like:
			___properties___ (this) { this.property = rhs ... }
		}

	*/

	this_tk       := synthetic_token(INSTANCE_NAME)
	named_this    := create_named_var_node(INSTANCE_NAME,true,true,0,loc)

	methods       := create_container_node()
	container     := create_container_node()

	properties_n  := ctx_amake(StringMap)
	...
	for not_is_eof() && has_not_error() && !at(.Close_Brace)
	{
		...
		{
			advance()
			expect(.Identifier,"expected a property name .")

			property := peek_ptk().text
			rhs      : ^Node 
			...
			//
			skip_stmt_terminator()
			compile_error_cond(property not_in properties_n,"Property '%v' is duplicate in Class '%v'.",property,class_name_tk.text)
				
			class_declare_property(property)
			compile_error_cond(field_count < MAX_FIELDS,"cannot have more than '%v' fields, but got '%v'.",MAX_FIELDS,field_count) 
				
			properties_n[property] = -1
			field_count           += 1

			set := create_set_property_node(property,named_this,rhs,loc)
			container_node_append(container,create_expression_node(set,loc))
			continue
		}
	...
	// Nota(jstn): evitamos disperdicio de memoria 
	if field_count > 0 do container_node_append(methods,parse_properties_method(container))

	expect(.Close_Brace," expected '}' after class body.")
	return create_class_node(class_name_tk.text,extends,field_count,super,methods,properties_n,loc)
}
...
parse_method :: proc() -> ^Node
{
	...
	if mtype
	{
	    ...
	    m_return  = create_return_node(scope_get_local_count(),create_named_var_node(INSTANCE_NAME,true,kind_can_assign(ldata.kind),ldata.index,loc),loc)
	}
...
parse_properties_method :: proc(c: ^Node) -> ^Node
{
	loc := peek_cloc()
	method_name_tk := synthetic_token(CLASS_PROPERTIES)
	...
	// body      := create_block_node(0,create_container_node(),loc)
	m_return  := create_default_return_node(1,loc)

	// container_node_append(c,body)

	// Nota(jstn): é oconstructor da class, não pode retornar nada
	// e devemos garantir que retorne a instancia

	// Nota(jstn): para sair do metodo
	container_node_append(c,m_return)

	sbody     := create_block_node(1,c,loc)
	return create_function_node(name,argc,sbody,loc)
}
...
for_stmt :: proc( bp: BindPower ) -> ^Node
{
	...
	/*
...

		{
			...
			expr_t.cleanup(value_t) // para limpar dados, caso algum tipo tenha alocado
		}
	*/
	...
	container_args := create_container_node()
	container_node_append(container_args,get_value_t)

	// (1) value_t = expr_t.iterate(value_t)
	...
	call_node_1 := create_call_method_node(".next",get_expr_t,container_args,loc)
	...
	// Não é uma expressão, pois o while vai tratar dele.
	call_node_2 := create_call_method_node(".end",get_expr_t,container_args,loc)
	...
	cleanup     := create_expression_node(create_call_method_node(".cleanup",get_expr_t,container_args,loc),loc)

	// 
	container_node_append(hidden_body,while_node)
	container_node_append(hidden_body,cleanup)
...
repeat_stmt :: proc( bp: BindPower ) -> ^Node
{
	loc := peek_cloc(); advance()
	...
	*/
	...
	skip_for(.Newline,1)
	expect(.Until,"expected 'until' after repeat body.")

	condition     := expression(.DEFAULT)

	//  
	cond_t        := synthetic_token(".cond_t")
	scope_declare_entety(cond_t,.IDENTIFIER,loc)

	// gets
	_,ldata_c     := scope_check_redeclaration_var(cond_t)
	get_cond_t    := create_named_var_node(cond_t.text,true,kind_can_assign(ldata_c.kind),ldata_c.index,loc)

	// cond_t = !expr_t
	cond          := create_unary_node(.OP_NOT,create_unary_node(.OP_BOOLIANIZE,condition,loc),loc)
	cond_t_negate := create_expression_node(create_assignment_node(cond_t.text,true,ldata_c.index,cond,loc),loc)

	while_node    := create_while_node(get_cond_t,body,loc)
	hidden_body   := create_container_node() 

	block_append_node(body,cond_t_negate)

	container_node_append(hidden_body,create_stmt_node(create_definevariable_node(cond_t.text,true,create_literal_node(.OP_TRUE,loc),loc)))
	container_node_append(hidden_body,while_node)
...
return_stmt :: proc(bp: BindPower) -> ^Node
{
	...
	if is_init_function()
	{
	    ...
	    return create_return_node(l_count,create_named_var_node(INSTANCE_NAME,true,kind_can_assign(ldata.kind),ldata.index,loc),loc)
	}
	...
	else do expr =  expression(bp) 

	return create_return_node(l_count,expr,loc)
}


with :: proc(bp: BindPower) -> ^Node 
{
	loc       := peek_cloc() ; advance()
	obj       := expression(bp)
	...
	expect(.Open_Brace,"expected '{{' after with.")
	skip_for(.Newline,-1)
	...
	for not_is_eof() && has_not_error() && !at(.Close_Brace)
	{
		container_node_append(container,create_expression_node(member(obj,.DEFAULT),peek_cloc()))
		skip_for(.Newline,-1)
		skip_stmt_terminator()
	}

	// container_node_append(container,create_literal_node(.OP_NIL,loc))

	skip_stmt_terminator()
	expect(.Close_Brace,"expected '}}' after with body.")
	return  container
}