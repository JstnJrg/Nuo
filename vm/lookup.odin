...
@(cold,private = "file")
nud :: proc "contextless" (type: TokenType,callable: call_nud, power : BindPower = .PRIMARY) 
{
	cludata     := &current_lookup.lu_map[type]
	cludata.bp   = power
	cludata.nbp  = power
...
register_lookups 	:: proc()
{
	...
	nud(.Add,    unary,.UNARY) 
	nud(.Not,    unary,.UNARY) 
	nud(.Not_Literal,unary,.UNARY)
	nud(.Not_Bit,unary,.UNARY)
