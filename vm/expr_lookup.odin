...
binary   :: proc(lhs: ^Node, bp: BindPower) -> ^Node 
{
	...
	{
		case .Sub:  return fold(create_binary_node(.OP_SUB,lhs,expression(bp),loc))
		case .Add:  return fold(create_binary_node(.OP_ADD,lhs,expression(bp),loc))
		case .Mult: return fold(create_binary_node(.OP_MULT,lhs,expression(bp),loc))

		case .Mult_Mult:   return fold(create_binary_node(.OP_MULT_MULT,lhs,expression(bp),loc))
		case .Quo:         return fold(create_binary_node(.OP_DIV,lhs,expression(bp),loc))
		case .Mod:         return fold(create_binary_node(.OP_MOD,lhs,expression(bp),loc))
		case .And_Bit:     return fold(create_binary_node(.OP_BIT_AND,lhs,expression(bp),loc))
		case .Xor_Bit:     return fold(create_binary_node(.OP_BIT_XOR,lhs,expression(bp),loc))
		case .Or_Bit:      return fold(create_binary_node(.OP_BIT_OR,lhs,expression(bp),loc))
		case .Shift_L_Bit: return fold(create_binary_node(.OP_SHIFT_LEFT,lhs,expression(bp),loc))
		case .Shift_R_Bit: return fold(create_binary_node(.OP_SHIFT_RIGHT,lhs,expression(bp),loc))
		case .Equal_Eq:    return fold(create_binary_node(.OP_EQUAL,lhs,expression(bp),loc))
		case .Less: 	   return fold(create_binary_node(.OP_LESS,lhs,expression(bp),loc))
		case .Greater: 	   return fold(create_binary_node(.OP_GREATER,lhs,expression(bp),loc))
		case .Not_Eq: 	   return fold(create_binary_node(.OP_NOT_EQUAL,lhs,expression(bp),loc))
		case .Less_Eq:     return fold(create_binary_node(.OP_LESS_EQUAL,lhs,expression(bp),loc))
		case .Greater_Eq:  return fold(create_binary_node(.OP_GREATER_EQUAL,lhs,expression(bp),loc))

		case .And: 		   return fold(create_binary_node(.OP_AND,create_unary_node(.OP_BOOLIANIZE,lhs,loc),create_unary_node(.OP_BOOLIANIZE,expression(bp),loc),loc))
		case .Or :		   return fold(create_binary_node(.OP_OR,create_unary_node(.OP_BOOLIANIZE,lhs,loc),create_unary_node(.OP_BOOLIANIZE,expression(bp),loc),loc))
...
primary :: proc(bp: BindPower) -> ^Node
{
	...
	{
		case .True :  
			return create_literal_node(.OP_TRUE,loc)
		case .False:  
			return create_literal_node(.OP_FALSE,loc)
		case .Null :  
			return create_literal_node(.OP_NIL,loc)
		case .Int  :  
			value, _ := parse_int(t.text)
			return create_int_node(value,loc)
		case .Float:  
			value, _ := parse_f64(t.text)
			return create_float_node(Float(value),loc)
		case .String: 
			return create_string_node(t.text,loc)
		case : 
			unreachable("Oops! it's a compiler bug ."); return nil
	}
}
...
identifier :: proc(bp: BindPower) -> ^Node 
{
	...
	if is_in_class() && identifier_c(bp,var_name_tk,&l_node,loc) do return l_node
...
identifier_c :: proc(bp: BindPower, var_name_tk: Token ,l_node: ^^Node,loc: Localization) -> (sucess: bool)
{
	class_has_property(var_name_tk) or_return
	this_tk    := synthetic_token(INSTANCE_NAME)


	// Nota(jstn): está em um método que não seja o inicializador de propriedades
	if is_in_function()
	{
		// has, pdata := scope_check_redeclaration_var(this_tk)
		// compile_error_assert(has,"Oops! this should not happen. It's a compiler bug in 'identifier_c' handler.")

		// this_named := create_named_var_node(this_tk.text,true,kind_can_assign(pdata.kind),pdata.index,loc)
		// l_node^     = create_get_property_node(var_name_tk.text,this_named,loc)

		// create_get_property_node(property,lhs,loc)
		// create_set_property_node(property,lhs,binary_node,loc)
		return false
	}

	// NotaI(jstn): está na função propriedade, this tem indice 0
	this_named := create_named_var_node(this_tk.text,true,true,0,loc)
	l_node^     = create_get_property_node(var_name_tk.text,this_named,loc)
	return  true
}
...
super :: proc(bp: BindPower) -> ^Node
{
	...
	has,ldata :=  scope_check_redeclaration_var(synthetic_token(INSTANCE_NAME))
	...
	lhs         := create_named_var_node(INSTANCE_NAME,true,kind_can_assign(ldata.kind),ldata.index,loc)
...
{
	loc   := peek_cloc() ; advance()
	compile_error_cond(is_in_class(),"can't use '%v' outside of a class.",INSTANCE_NAME)
	...
	return create_named_var_node(INSTANCE_NAME,true,kind_can_assign(ldata.kind),ldata.index,loc)
}


callable :: proc(bp: BindPower) -> ^Node
{
	loc   := peek_cloc(); advance()
	return create_named_var_node(peek_ptk().text,false,false,-1,loc)
}



task :: proc(bp: BindPower) -> ^Node
{
	loc   := peek_cloc(); advance()
	return create_named_var_node(peek_ptk().text,false,false,-1,loc)
}
...
complex     :: proc(bp: BindPower) -> ^Node 
{
	loc   := peek_cloc(); type := peek_ctype(); advance()
	if at(.Dot) do return create_named_var_node(peek_ptk().text,false,false,-1,loc)

	container := create_container_node()
	expect(.Open_Paren,"expected '(' after Complex.")
	...
	for not_is_eof() && has_not_error() && !at(.Close_Paren)
	{
		...
		skip(.Comma)
		 ...
		 expect(.Close_Paren,"expected ')' after Complex arguments.")
	argc := get_container_node_len(container)

	compile_error_cond(argc == 2, "Complex expects 2 arguments, but got %v",argc)
	return create_dinamic_node(.COMPLEX,container,loc)
