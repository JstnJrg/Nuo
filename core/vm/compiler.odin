...
// ===================== Class
begin_class        :: proc "contextless" () { current_compiler.class_depth += 1 }
end_class          :: proc "contextless" () { current_compiler.class_depth -= 1}
...
// get_loop_scope_depth                :: proc "contextless" () -> int { return current_compiler.loop_data.scope_depth }   
...
// 	for name,localdata in cscope.locals do println("var name: ",name,"| stack index: ",localdata.index)
// }
...
{
	callable_c :: proc(_: int, _: int, class_name : string)       { loc := synthetic_loc(); global_declare_entety(class_name,.CLASS,loc) }
...
// global_declare_entety_clone :: proc "contextless" (var_name : Token,etype: EntetyType ,loc: Localization) 
// {
// 	sd       : SymbolData
...
// 	sd.index   = -1
// 	sd.kind    = etype
// 	_var_name := string_clone(var_name.text,ctx_allocator())
	
// 	current_compiler.globals.symbols[_var_name] = sd
// }
