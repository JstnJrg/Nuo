...
{
	cs := ctx.call_state
	if ctx.runtime_error(cs.argc == 1 && ( IS_VARIANT_PTR(ctx.peek_value(0),.OFUNCTION) || IS_VARIANT_PTR(ctx.peek_value(0),.NFUNCTION) ) ,"'connect' expects %v, but got %v and must be 'Callable'.",1,cs.argc) do return	
...
{
	cs := ctx.call_state
	if ctx.runtime_error(cs.argc == 1 && ( IS_VARIANT_PTR(ctx.peek_value(0),.OFUNCTION) || IS_VARIANT_PTR(ctx.peek_value(0),.NFUNCTION) ) ,"'disconnect' expects %v, but got %v and must be 'Callable'.",1,cs.argc) do return	
...
emit :: proc(ctx: ^Context)
{
	...
	//
	__append(adata,cs.args[:])
	what   := adata[0:][:argc]

	defer { cs.args = cs.args[0:][:0]; unref_object(args) }
...
// {
// 	cs    := ctx.call_state
// 	if ctx.runtime_error(cs.argc == 1 && ( IS_VARIANT_PTR(ctx.peek_value(0),.OFUNCTION) || IS_VARIANT_PTR(ctx.peek_value(0),.NFUNCTION) ),"'sort_costum' expects %v, but got %v and must be 'Callable'.",1,cs.argc) do return
...
//         ctx.copy_args(cs.args[1:],args[:],1)
//     defer 
//     {
...
// {
// 	cs    := ctx.call_state
// 	if ctx.runtime_error(cs.argc == 1 && ( IS_VARIANT_PTR(ctx.peek_value(0),.OFUNCTION) || IS_VARIANT_PTR(ctx.peek_value(0),.NFUNCTION) ),"'map' expects %v, but got %v and must be 'Callable'.",1,cs.argc) do return
...
// {
// 	cs    := ctx.call_state
// 	if ctx.runtime_error(cs.argc == 1 && ( IS_VARIANT_PTR(ctx.peek_value(0),.OFUNCTION) || IS_VARIANT_PTR(ctx.peek_value(0),.NFUNCTION) ),"'filter' expects %v, but got %v and must be 'Callable'.",1,cs.argc) do return
...
//           ctx.copy_args(cs.args[1:],args[:],1)
//     defer ctx.copy_args(args[:],cs.args,1)
...
// {
// 	cs    := ctx.call_state
// 	if ctx.runtime_error(cs.argc == 2 && ( IS_VARIANT_PTR(ctx.peek_value(0),.OFUNCTION) || IS_VARIANT_PTR(ctx.peek_value(0),.NFUNCTION) ),"'reduce' expects %v, but got %v and must be 'Callable' and 'Variant'.",2,cs.argc) do return
...
//     ctx.copy_args(cs.args[1:],args[:],2)
//     defer 
//     {
//     	cs.args = cs.args[0:][:0] // garantimos que não aconteca unref duas vezes
//     	unref_slice(args[:])
...
// {
// 	cs    := ctx.call_state
// 	if ctx.runtime_error(cs.argc == 1 && ( IS_VARIANT_PTR(ctx.peek_value(0),.OFUNCTION) || IS_VARIANT_PTR(ctx.peek_value(0),.NFUNCTION) ),"'find_costum' expects %v, but got %v and must be 'Callable'.",1,cs.argc) do return
