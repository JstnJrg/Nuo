...
{
	a: for op in Opcode(0)..< Opcode.OP_OPERATOR_COUNT
	{
		b: for type_a in VariantType do for type_b in VariantType 
		{			
			if get_op_binary(op,type_a,type_b) != nil do continue 
			if (type_a == .MAX ) || (type_b == .MAX)  do continue 
			...
			#partial switch op
			{
				...
				case .OP_NOT_EQUAL : register_op(op,type_a,type_b,variant_variant_not_equal);  continue	
			}
...
variant_variant_fallback :: #force_inline proc(variant_left,variant_right,variant_return: ^Value, ctx: ^Context) {
	if   ctx.is_trace_error() do ctx.runtime_error(false,"invalid operation between '%v' and '%v' .",GET_VARIANT_TYPE_NAME(variant_left),GET_VARIANT_TYPE_NAME(variant_right))
...
variant_variant_unary_fallback :: #force_inline proc(_,variant_right,variant_return: ^Value, ctx: ^Context) {
	if ctx.is_trace_error() do ctx.runtime_error(false,"unary operation not supported for type '%v'.",GET_VARIANT_TYPE_NAME(variant_right))	
	BOOL_VAL_PTR(variant_return,false)
}
...
variant_variant_not_equal :: #force_inline proc(variant_left,variant_right,variant_return: ^Value, ctx: ^Context) 
{
	_bool := variant_hash_compare(variant_left,variant_right,0)
	if ctx.gc_allowed { unref_value(variant_left); unref_value(variant_right) }
	BOOL_VAL_PTR(variant_return,!_bool)
