...
{
	 ctx := get_context()
	_gc   = new(GC,ctx.get_runtime_allocator()); ctx.runtime_assert(_gc != nil,"_gc != nil condition is false.")
} 
...
ref_value   :: proc (value: ^Value)
{
	...
	AS_NUOOBECT_PTR(value).ref_count += 1
}

ref_value_costum :: proc (value: ^Value, offset: int)
{
	if value.type < .OBJ do return
	AS_NUOOBECT_PTR(value).ref_count += offset
}
...
ref_object   :: proc (obj: ^NuoObject) { obj.ref_count += 1 }
...
unref_slice         :: proc (s: []Value) { for &value in s do unref_value(&value) }

unref_table         :: proc (t: ^Table)  { for _,&p in t do    unref_value(&p) }
...
gc_free_obj_with_dependencies :: proc(obj: ^NuoObject)
{
	#partial switch obj.type
	{
		...
		case .INSTANCE:
			...
			if sucess
			{
				...
				*/
				...
				ctx.runtime_assert(obj_instance.ref_count > 0,"_deinit, ref_count reachs zero ? Oops it's a bug.")

				// Nota(jstn): caso seja diferente de 1, então existe mais uma referencia a esse objecto
				// então removemos a nossa referencia (+1)

				obj_instance.ref_count -= 1
				if obj_instance.ref_count > 0 do return
			}
	    ...
	    case .CALLABLE:
	    	...
	    	_obj := reinterpret_object(obj,NuoCallable)

	    	unref_value(&_obj.callable)
	    	unref_slice(_obj.args[:])
		...
		case .TASK:
	    	...
	    	_obj     := reinterpret_object(obj,NuoTask)
	    	task     := _obj.task

	    	tos := (task.tos-1) > 0 ? task.tos-1: 0
	    	unref_slice(task.stack[:tos])
	    	unref_value(&_obj.callable)
...
gc_free_object :: proc(obj: ^NuoObject)
{
	...
	#partial switch obj.type
	{
		...
		case .CALLABLE:
			...
			obj_callable := reinterpret_object(obj,NuoCallable)
			delete(obj_callable.args)
			free(obj_callable)

		case .TASK:

	    	obj_task  := reinterpret_object(obj,NuoTask)
	    	task      := obj_task.task
