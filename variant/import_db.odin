...
ImportApi  :: enum u8
{
	...
	IMPORT,  // requerem importação, ao serem importadas, são inicializadas pelo script
	BUILTIN, // não requerem importação e são visiveis a todos

	NUO      // é a tabela de global da VM, não podem ser importado, é o contexto inicial da VM
}
...
import_db_register_init  :: proc(iid : int ,fn : ImportInit) { import_db_get_iinfo(iid).init_fn = fn }


import_db_load_module    :: proc(iid: int)
{

	/* 
		Nota(jstn): se foi carregado, então ou são nativas, ou um modulo mesmo carregado,
		então, não é necessario verificar o tipo por um switch case.
	*/
	
	if import_db_was_loaded(iid) do return
	if fn, ok := import_get_init_fn(iid); ok do fn(iid)
}


import_get_init_fn      :: proc(iid: int) -> (ImportInit,bool)
{
	fn := import_db_get_iinfo(iid).init_fn
	ok := fn != nil
	return fn, ok
}

import_db_was_loaded    :: proc "contextless" (iid: int) -> bool 
{ 
	iinfo := import_db_get_iinfo(iid)
	return true if iinfo.api != .IMPORT else iinfo.loaded
}

import_db_set_loaded     :: proc "contextless" (iid: int, value: bool) { import_db_get_iinfo(iid).loaded = value }
...
import_db_reserve        :: proc (iid, amount: int)   { reserve(import_db_get_globals(iid),amount) }
