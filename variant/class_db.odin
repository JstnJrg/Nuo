...
ClassApi     :: enum 
{
	ATOMIC,  // Nota(jstn): não se pode extender dessas classes, não possui metainformação	
	USER,    // Nota(jstn): é registado somente no gloabal local
	SHARED,  // Nota(jstn): visivel para todas as globais
} 

PropertyTypeHash :: enum u32
{
	NORMAL     = 0, //Não recebem nenhuma mascara
	NUO        = 0x99999999,
	STATIC     = 0x11111111,
	INSTANCE   = 0x22222222,
	INTRINSICS = 0x33333333,
}
...
PropertyInfo     :: struct
{
	...
	sgetter : FnClassSetterGetter,
	ssetter : FnClassSetterGetter,
}
...
FnCreator           :: proc(ctx: ^Context)
ClassInit           :: proc(cid: int)
...
class_db_registe_instrinsics :: proc(cid: int)
{
	// Nota(jstn): não possuem meta informação
	if class_db_check_api(cid,.ATOMIC) do return
	
	CLASS_B_METHOD(cid,"get_meta",instance_get_meta,.INSTANCE)
	CLASS_B_METHOD(cid,"set_meta",instance_set_meta,.INSTANCE)
	CLASS_B_METHOD(cid,"has_meta",instance_has_meta,.INSTANCE)
	CLASS_B_METHOD(cid,"call",instance_call,.INSTANCE)
}



class_db_register_class :: proc(class_name : string, sid := -1, capi : ClassApi = .USER, creator := default_class_creator ) -> int 
{
	...
	// 
	class_db_register_class_method(cid,"new",ODINFUNCTION_VAL(creator),.STATIC)
	class_db_registe_instrinsics(cid)

	return cid
}

class_db_methods_table_reserve :: proc(cid, size : int)
{
	instrinsic_count := class_db_check_api(cid,.ATOMIC)? 0:3
	ci               := db_get_classinfo(cid)
	reserve(&ci.methods,size+instrinsic_count)
}

class_db_load :: proc (cid: int, fn : ClassInit) { fn(cid) }
...
class_db_check_api              :: proc "contextless" (cid: int, api: ClassApi) -> bool { return db_get_classinfo(cid).api == api }
...
class_db_get_method_list_recursively :: #force_inline proc (cid: int, m_hash: u32, mtype : MethodType, allocador: Allocator) -> []Value 
{ 
	list := make([dynamic]Value,allocador)
	m    : Value

	if class_db_get_method(cid,m_hash,&m,mtype) do append(&list,m)
	...
	{
		if class_db_get_method(sid,m_hash,&m,mtype) do append(&list,m)
...
{ 
	...
	return ok ? pi.setter : class_db_default_error
}
...
{ 
	...
	return ok ? pi.getter : class_db_default_error
}

class_db_get_csetter             :: #force_inline proc (cid: int, hash: u32) -> FnClassSetterGetter  
{ 
	ci     := db_get_classinfo(cid)
	hash   := hash ~ u32(PropertyTypeHash.STATIC)
	...
	return ok ? pi.ssetter : class_db_default_csetter
}
...
{ 
	...
	return ok ? pi.sgetter : class_db_default_cgetter
}




class_db_get_classes_names      :: proc(data: int , callable : proc(data,cid: int,cname: string)) 
{
	for i in 0..< db_get_classinfo_id()
	{ 
		if class_db_check_api(i,.ATOMIC) || class_db_check_api(i,.SHARED) do callable(data,i,class_db_get_class_name(i))
	}
} 
...
{
	table_instance_set(&instance.fields,string_db_get_hash(sid),ret)
}

class_db_default_error  :: proc(instance: ^NuoInstance, sid: int, ret: ^Value, ctx: ^Context) 
{
	ctx.runtime_error(false,"invalid acess to property or key '%v' on instance of '%v'.",string_db_get_string(sid),class_db_get_class_name(instance.cid))
}
...
class_db_default_getter  :: proc(instance: ^NuoInstance, sid: int, ret: ^Value, ctx: ^Context)
{
    ...
    if table_instance_get(&instance.fields,hash,ret) do return 

    // Nota(jstn): precisamos de uma maneira do metodo lembrar a instancia a que está associada
    // precisamos encapsular a instancia.
    if class_db_get_method(instance.cid,hash,ret,.INSTANCE)
    {
    	this    := NUOOBJECT_VAL(instance)
    	method  := ret
    	NUOOBJECT_VAL_PTR(ret,create_bound_method(&this,method))
    	return
    }
}


class_db_default_csetter  :: proc(cid, sid: int, ret: ^Value, ctx: ^Context) { ctx.runtime_error(false,"cannot find member '%v' in '%v' Class.",string_db_get_string(sid),class_db_get_class_name(cid)) }

class_db_default_cgetter  :: proc(cid, sid: int, ret: ^Value, ctx: ^Context) { ctx.runtime_error(false,"cannot find member '%v' in '%v' Class.",string_db_get_string(sid),class_db_get_class_name(cid)) }
...
class_db_get_deinit_method     :: proc (instance: ^NuoInstance) -> (Value,bool)
{
	...
	sucess := class_db_get_method(instance.cid,string_db_get_hash(sid),&method,.INSTANCE)
