...
register_operators :: proc()
{
	...
	for type in VariantType do register_op(.OP_SHIFT_LEFT,.ARRAY,type,array_variant_shift_left)
}
...
array_variant_shift_left :: proc(lhs,rhs,ret: ^Value,ctx: ^Context)
{
	array0  := AS_NUOOBECT_PTR(lhs,NuoArray)
	data0   := &array0.data

	// ref_value(rhs); ao carregar já foi incrementado o rf
	append(data0,rhs^)
    unref_value(lhs)
