...
_stdin :: proc(ctx: ^Context) 
{
	cs   := ctx.call_state

	if ctx.runtime_error(cs.argc == 0,"'stdin' expects %v, but got %v.",0,cs.argc) do return
    if ctx.runtime_warning(stdin != nil,"stdin is not available.") { NIL_VAL_PTR(cs.result); return }
...
_stdout :: proc(ctx: ^Context) 
{
	cs   := ctx.call_state

	if ctx.runtime_error(cs.argc == 0,"'stdout' expects %v, but got %v.",0,cs.argc) do return
    if ctx.runtime_warning(stdin != nil,"stdout is not available.") { NIL_VAL_PTR(cs.result); return }
...
clean_path :: proc(ctx: ^Context) 
{
	cs   := ctx.call_state
	if ctx.runtime_error(cs.argc == 1 && IS_VARIANT_PTR(ctx.peek_value(0),.STRING),"'clean_path' expects %v, but got %v and must be '%v'.",1,cs.argc,GET_TYPE_NAME(.STRING)) do return
	...
	s,err := os2.clean_path(path,ctx.get_runtime_allocator())
	ctx.runtime_warning( err == nil ,os2.error_string(err))
...
is_absolute_path :: proc(ctx: ^Context) 
{
	cs   := ctx.call_state
	if ctx.runtime_error(cs.argc == 1 && IS_VARIANT_PTR(ctx.peek_value(0),.STRING),"'is_absolute_path' expects %v, but got %v and must be '%v'.",1,cs.argc,GET_TYPE_NAME(.STRING)) do return

	path := to_string(AS_NUOOBECT_PTR(ctx.peek_value(0),NuoString))
    BOOL_VAL_PTR(cs.result,os2.is_absolute_path(path))
}
...
close :: proc(ctx: ^Context) 
{
	...
	is_std     := (file == stdin || file == stdout || file == stderr )
